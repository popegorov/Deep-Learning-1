...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            s = self.input_mean*self.input_mean
            self.var = np.mean(s, axis=0)
            ...
            self.running_mean = self.running_mean*(1-self.momentum) + self.mean*self.momentum
            self.running_var = self.running_var*(1-self.momentum) + self.var*self.momentum*(batch_size/(batch_size-1))
        else:
        ...
        if self.affine and self.bias is not None:
            ...
            return self.weight * self.norm_input
        else:
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        """
        if self.affine:
            dx = grad_output * self.weight
        else:
            dx = grad_output

        if self.training:
            dt = np.sum(dx*self.input_mean, axis=0)
            dr = -dt*(self.inv_sqrt_var *self.inv_sqrt_var)
            dsig = dr*(1/2*self.inv_sqrt_var)
            ds = (1/input.shape[0])*dsig
            dz = dx*self.inv_sqrt_var + 2*ds*self.input_mean
            dm = -np.sum(dz, axis=0)
            return dz + (dm*1/input.shape[0])
        else:
            return dx / np.sqrt(self.running_var + self.eps)
...
class Dropout(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        return 1/(1-self.p)*self.mask*input if self.training else input
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        :return: array of the same size
        """
        return grad_output*1/(1-self.p)*self.mask if self.training else grad_output
...
class Sequential(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        """
        self.prev_output = []
        self.prev_output.append(input)
        for i, module in enumerate(self.modules):
            self.prev_output.append(module.compute_output(self.prev_output[i]))
        return self.prev_output.pop()
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        for i in range (len(self.modules) - 1, -1, -1):
            prev_output = self.prev_output[i]
            prev_grad = self.modules[i].backward(prev_output, prev_grad)
